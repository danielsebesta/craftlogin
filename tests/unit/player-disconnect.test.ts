...
const chatComponentSchema = z.object({ text: z.string(), color: z.string() });
...
describe('Minecraft disconnect delivery', (): void => {
  ...
  it.each([
    ['success', '#A2D060'],
    ['error', '#F47B74'],
  ] as const)('paints a post-login kick %s', async (tone, color): Promise<void> => {
    const delivery = await deliverDisconnect('after-login', tone);

    expect(delivery.packet).toBe('kick_disconnect');
    expect(delivery.text).toBe(message);
    expect(delivery.color).toBe(color);
  });

  it('paints a pre-login rejection neutral by default', async (): Promise<void> => {
    const delivery = await deliverDisconnect('before-login');
...
function extractComponent(packet: string, rawReason: unknown): { text: string; color: string } {
  ...
  return chatComponentSchema.parse(parsedReason);
}
